...
class TernaryGateNN:
    ...
    def __init__(self, input_neurons: int, hidden1: int, hidden2: int, 
                 hidden3: int, output_neurons: int, lr: float = 0.01):
        ...
        # Initialize weights
        self.weights = {
            'w1': np.random.randn(self.input_neurons, self.hidden1),
            'w2': np.random.randn(self.hidden1, self.hidden2),
            'w3': np.random.randn(self.hidden2, self.hidden3),
            'w4': np.random.randn(self.hidden3, self.output_neurons)
        }
    ...
    def set_weights(self, weights_data: Dict[str, Any]):
        ...
        if 'biases' in weights_data:
            ...
            # Add regularization
    def __init__(self, ..., dropout_rate=0.1, l2_reg=0.001):
